...
foreach ($filepath in $files) {
    ...
    if ($content -notmatch "docs\.comfy\.org/zh/") {
        ...
        if ($headerEndIdx -ge 0) {
            ...
            for ($i = 0; $i -le $headerEndIdx; $i++) { [void]$newLines.Add($lines[$i]) }
            [void]$newLines.Add($headerRefLine)
            for ($i = $headerEndIdx + 1; $i -lt $lines.Count; $i++) { [void]$newLines.Add($lines[$i]) }
    ...
    for ($p = 0; $p -lt $parts.Count; $p++) {
        if ($parts[$p] -match "^```") { continue }
        
        foreach ($trans in $nodeTranslations) {
            $engName = $trans.eng
            $chnName = $trans.chn
            
            if ($annotated.ContainsKey($engName)) { continue }
            if (($engName -eq "CLIP Text Encode" -or $engName -eq "CLIP Text Encoder") -and $clipDone) { continue }
            
            $idx = $parts[$p].IndexOf($engName)
            if ($idx -eq -1) { continue }
            
            $afterStart = $idx + $engName.Length
            $afterText = ""
            if ($afterStart -lt $parts[$p].Length) {
                $len = [Math]::Min(30, $parts[$p].Length - $afterStart)
                $afterText = $parts[$p].Substring($afterStart, $len)
            }
            ...
            if ($afterText -match "（" -and $afterText -match "）") {
                ...
                if ($closeIdx -gt $parenIdx -and $closeIdx -lt 25) {
                    ...
                    if ($engName -eq "CLIP Text Encode" -or $engName -eq "CLIP Text Encoder") { $clipDone = $true }
                    continue
                }
            }
            
            $annotation = "（" + $chnName + "）"
            $parts[$p] = $parts[$p].Substring(0, $afterStart) + $annotation + $parts[$p].Substring($afterStart)
            $annotated[$engName] = $true
            if ($engName -eq "CLIP Text Encode" -or $engName -eq "CLIP Text Encoder") { $clipDone = $true }
        }
    }
